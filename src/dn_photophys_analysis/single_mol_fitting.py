...
@jit(nopython=True, nogil=True, cache=False)
def get_loc_prec(crlb: 'np.ndarray') -> float:

    return (np.sqrt(crlb[0]) + np.sqrt(crlb[1])) / 2

@jit(nopython=True, nogil=True, cache=False)
def calc_fisher_im(spot, theta_opt):
...
def calc_loc_prec(spot, theta_opt):
    ...
    fisher_matrix = calc_fisher_im(spot, theta_opt)

    crlb = get_crlb(fisher_matrix)

    return get_loc_prec(crlb)
...
def gaussmle(spots, eps, max_it, mode='slow'):
    ...
    for i in prange(n):
        ...
        loc_prec[i, 0] = loc_prec_calc(spot, thetas[i, :])
