...
def local_maxima_cv(img, threshold, neighborhood=8):
...
def dog_filter(image):
    ...
    filt_im = difference_of_gaussians(image, 1, 6)

    return filt_im

def opencv_dog(image):
...
def extract_spot(image, edges):
    ...
    horizontal_filt = image[:, edges[0, 0]:edges[0, 1]]
...
def get_spots(image_frame: 'np.ndarray', pix_res: float) -> list:
    ...
    smoothed_im = opencv_dog(image)

    threshold = np.sqrt(np.mean(smoothed_im ** 2))

    local_maxima = local_maxima_cv(smoothed_im, 3.5 * threshold)
...
def get_spots_denoised(denoised_im: 'np.ndarray', noisy_im: 'np.ndarray', pix_res: float) -> 'np.ndarray':
    ...
    image = denoised_im.view()
    noisy_image = noisy_im.view()

    smoothed_im = opencv_dog(noisy_image)
